...
- 真人录音统一加「真人测试_」前缀（两种命名均可加），如 `真人测试_第1周_问题1_20260906.m4a` 或 `真人测试_录音01_童年.wav`，与模拟测试文件互不覆盖
- 老爸自传模式（真人实测）：本地 `老爸录音/` 目录下所有 `.wav`/`.mp3`/`.m4a`（原始录音仅保存在本地，不进 git、不上传公开仓库），按文件名排序即录音顺序

## 输出
- 路径：`tests/transcripts/`
- 命名：与录音一一对应，如 `第1周_问题1_测试长者001_转写.md`
- 真人录音输出加同一前缀，如 `真人测试_第1周_问题1_转写.md`
- 老爸自传模式输出：`老爸自传_第1段_转写.md` … `老爸自传_第6段_转写.md`（按录音顺序 1-6）
- 格式：Markdown，按时间分段，每段带 `[MM:SS]` 时间戳

## 转写规则（严格遵循《语气保留指南》第一步）
1. 逐字转写：口头禅（嗯、啊、那个、对喽、哎呦）、语气词、重复的话、说一半的话，全部保留；
2. 不修正语法、不润色、不删除跑题内容；
3. 停顿用「……」标注（段间停顿超过约 0.8 秒时插入停顿行；真人老年语音慢语速、停顿多，阈值由 1.2 秒收紧到 0.8 秒）；
4. 笑声/叹气等情绪用方括号标注，如「[笑]」「[叹气]」（CrisperWhisper 无情绪检测时，由人工校对补齐）；
5. 方言词、口音特色词原样保留，可加小括号注释；
6. 无法可靠识别的字符标「□[?]」，文末附转写信息：模型、语言、音频时长、转写耗时、汉字数、待复核 [?] 数量。

## 参数（CrisperWhisper 2.0 Python API）
- **真人语音参数优化（v4）**：
  - 强制语言 `language="zh"`（脚本已固定）；
  - 自动标点、断句：由 Whisper 模型自带能力输出，不做二次加工；
  - 慢语速/多停顿优化：`PAUSE_SEC = 0.8`（停顿标注阈值从 1.2 秒收紧到 0.8 秒）；
  - 语气保留：`mode="verbatim"` 逐字转写，口头禅、语气词、重复、说一半的话全保留；不修正语法、不润色、不删除跑题内容；
  - 停顿标「……」、笑声标「[笑]」、叹气标「[叹气]」（情绪标签由英文标签自动换算，无情绪检测时人工校对补齐）。
...
- 后端开关：本地运行可用环境变量 `CRISPER_BACKEND`（`transformers`/`ct2`）与 `CRISPER_COMPUTE`（`float32`/`int8`）覆盖；默认 `transformers` + `float32`，与 GitHub Actions 保持一致
- 英文情绪标签自动换算：`[laughter]`→`[笑]`、`[sigh]`→`[叹气]`、`[cough]`→`[咳嗽]` 等；填充词 `[UM]`/`[UH]`→「嗯，」「啊，」；模型误报的非言语标签（`[crying]` `[scream]` `[sneeze]` 等）自动剔除
- 正文取 continuation 分段的 `result.chunks[].text`；`result.words` 仅用于行首时间戳与停顿（>0.8 秒补「……」）标注
- v3 修正（内测排障结论）：Whisper small 对生僻字按 UTF-8 字节级 token 解码，偶发字节不完整产生 U+FFFD，清洗时替换为「□」并在文末标注数量；空格分隔的连续语气词（如「啊， 啊， 啊，」）自动合并为一次；时间戳恢复 `[MM:SS]` 方括号格式
