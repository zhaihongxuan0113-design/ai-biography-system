...
- 正文取 continuation 分段的 `result.chunks[].text`（无乱码）；`result.words` 仅用于行首时间戳与停顿（>1.2 秒行尾补「……」）标注
- v3 修正（内测排障结论）：幻觉修复回卷会把 CJK 字节级 token 切半、产生 U+FFFD 乱码，故关闭 `hallucination_mitigation`；正文干净且保留跨段上下文（避免丢句）；空格分隔的连续语气词（如「啊， 啊， 啊，」）自动合并为一次
