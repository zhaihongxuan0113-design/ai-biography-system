...
- ✅ 每周 2 问自动发送已配置：`.github/workflows/weekly-question.yml`（每周一、周四北京时间 09:00 各建 1 个访谈 Issue；测试阶段不指派、不触发邮件通知）
...
- ✅ 真人实测（第 6 轮）：父亲自由讲述 6 段录音（约 48 分钟）已完成逐字转写：`tests/transcripts/老爸自传_第1-6段_转写.md`（共 2943 汉字；停顿「……」、[笑]/[叹气] 等情绪标注、待复核 [?] 位置已标出；原始录音仅本地保存不上传）
- ✅ 传记初稿已整合：`tests/stories/老爸自传_初稿.md` + `.txt`（6 章：童年与成长 / 军旅岁月 / 国企工作生涯 / 下海创业 / 房地产 / 人生感悟与寄语，共 7431 字，DeepSeek 严格按《语气保留指南》只理序不改词）
- ✅ 定制访谈提纲（老爸版）：`docs/定制访谈提纲_老爸版.md`（80 问，5 阶段各 16 问，第 51-90 周排期）+ 本地《定制访谈提纲_老爸版_排期表.xlsx》；`tests/questions.json` 已扩至 180 问
- ✅ 测试阶段邮件通知已关闭：仓库订阅 ignored + 历史 Issue 取消指派 + 工作流不再指派（定时发问保留）
- ⏳ 下一步：人工校对转写稿 [?] 处 → 按定制提纲继续访谈补素材 → 升级转写模型 → story-to-book 成书
...
4. 测试阶段已关闭邮件通知：每周一、周四请自行查看仓库 Issues 列表获取新问题（无需 Watch/邮箱提醒）。
